...
```{r summary data for active funds}
...
```{r Fees by Type of Active Fund Category}
...
```{r Fees by Type of Morningstar Fund Category (ACTIVE)}
...
```{r Fee changes among active share classes}
...
```{r Fund families with the lowest/highes fees across passive products}
...
write_csv(Passive_Summary,
          "data/results/passive/fees_activefunds_summary.csv")
...
```{r Fees by Type of Passive Fund Holdings}
...
```{r Fees by Share Class of Passive Fund Holdings}
...
```{r Fee changes among passive share classes}
...
```{r Fee changes among share classes (active & passive)}
...
```{r % change of share class fees from 2022 to 2023}
